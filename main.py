...
def get_viewshed_visualization(params: ViewshedQuery):
    ...
    fname = f"viewshed_{int(x)}_{int(y)}_{int(h)}_{int(r)}_{uuid.uuid4().hex[:6]}.png"
...
def get_viewshed_aw_visualization(params: ViewshedQuery):
    ...
    out_dir = Path(CONFIG["output_dir"])
    out_dir.mkdir(parents=True, exist_ok=True)
    fname = f"viewshed_aw_{int(x)}_{int(y)}_{int(h)}_{int(r)}_{uuid.uuid4().hex[:6]}.png"
    out_path = out_dir / fname
...
def get_viewshed_loop_visualization(params: ViewshedQuery):
    ...
    out_dir = Path(CONFIG["output_dir"])
    out_dir.mkdir(parents=True, exist_ok=True)
    fname = f"viewshed_loop_{int(x)}_{int(y)}_{int(h)}_{int(r)}_{uuid.uuid4().hex[:6]}.png"
    out_path = out_dir / fname
